...
#     option1 = st.selectbox("Choose X", ("displacement", "horsepower", "weight", "acceleration"))
#     option2 = st.selectbox("Choose Y", ("displacement", "horsepower", "weight", "acceleration"))
#     x, y = mpg[option1].values, mpg[option2].values

#     option3 = st.selectbox("Choose a Model", ("Line", "RBF-NN"))
#     if option3 == "Line":
#         option4 = st.slider("Choose Slope", min_value=-10., max_value=10.)
#         option5 = st.slider("Choose Intercept", min_value=-5*np.max(np.abs(y)), max_value=5*np.max(np.abs(y)))
#         st.pyplot(plot_lr(x, y, option4, option5, xlabel=option1, ylabel=option2))

#     if option3 == "RBF-NN":
#         option6 = st.slider("Center 1", min_value=1, max_value=x.shape[0])
#         option7 = st.slider("Center 2", min_value=1, max_value=x.shape[0])
#         option8 = st.slider("Choose Weight 1", min_value=-np.max(y)*5., max_value=np.max(y)*5.)
#         option9 = st.slider("Choose Weight 2", min_value=-np.max(y)*5., max_value=np.max(y)*5.)
#         option10 = st.slider("Choose Bandwidth", min_value=1e-4, max_value=np.median(y)/np.median(x)*5.)
#         st.pyplot(plot_rbf(x, y, option6 - 1, option7 - 1, option8, option9, option10, option1, option2))
# else:
#     x, y = created
#     option3 = st.selectbox("Choose a Model", ("Line", "RBF-NN"))
#     if option3 == "Line":
#         option4 = st.slider("Choose Slope", min_value=-10., max_value=10.)
#         option5 = st.slider("Choose Intercept", min_value=-5*np.max(np.abs(y)), max_value=5*np.max(np.abs(y)))
#         st.pyplot(plot_lr(x, y, option4, option5, xlabel='x', ylabel='y'))
...
#         option9 = st.slider("Choose Weight 2", min_value=-np.max(y)*5., max_value=np.max(y)*5.)
#         option10 = st.slider("Choose Bandwidth", min_value=1e-4, max_value=np.median(y)/np.median(x)*5.)
#         st.pyplot(plot_rbf(x, y, option6 - 1, option7 - 1, option8, option9, option10, 'x', 'y'))
